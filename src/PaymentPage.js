...
function PaymentPage() {
  ...
  // Normalize: uppercase the network so it matches keys
  const normalizedNetwork = network ? network.toUpperCase() : "";

  const handleSubmit = (e) => {
    e.preventDefault();
    if (!message.includes(REFERENCE_NUMBER)) {
      setError("⚠️ Invalid message. Please include the correct reference number.");
    } else {
      setError("");
  ...
  return (
    <div className="container">
      ...
      {/* Fraud Alert right under header */}
      ...
      {Array.isArray(ACCOUNTS[normalizedNetwork]) ? (
        ...
        <p className="error">⚠️ No accounts found for the selected network.</p>
      )}
